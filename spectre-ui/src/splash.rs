    ...
    pub fn get_fade_out_alpha(&self) -> f32 {
        ...
        if elapsed >= self.show_duration {
            ...
            if elapsed < fade_out_end {
                return 1.0
                    - ((elapsed - fade_out_start).as_secs_f32()
...
impl SplashScreen {
    pub fn new(ctx: &egui::Context) -> Self {
        ...
        println!(
            "[Spectre.dbg] Splash screen initialized (fade: {}ms, show: {}ms)",
            splash.fade_duration.as_millis(),
            splash.show_duration.as_millis()
        );
        splash
    }
        ...
        egui::Area::new(egui::Id::new("splash_area"))
            ...
            .show(ctx, |ui| {
                ...
                if let Some(ref texture) = self.banner_texture {
                    ...
                    let banner_size_vec = egui::vec2(
                        banner_size[0] as f32 / 2.0 * 1.03,
                        banner_size[1] as f32 / 2.0 * 1.03,
                    );
                    let banner_rect =
                        egui::Rect::from_center_size(screen_rect.center(), banner_size_vec);
    ...
    fn draw_orbit_loader(&self, ui: &egui::Ui, center_x: f32, center_y: f32, alpha: f32) {
        ...
        painter.circle_filled(egui::pos2(x1, center_y), dot_size1 / 2.0, color1);
        ...
        painter.circle_filled(egui::pos2(x2, center_y), dot_size2 / 2.0, color2);
    }
}
